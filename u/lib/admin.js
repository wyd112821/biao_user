...
layui.define('view', function (exports) {
	var $ = layui.jquery
		...
		, admin = {
			...
			, navdone:function(){
				if($('.layui-nav .layui-this').length<=0){
					var pathURL = admin.correctRouter(layui.router().path.join('/'));
					var othisx=$('.layui-nav dd[data-qt*="'+pathURL.substr(1)+'"]');
					if(othisx.length>0){
						othisx.addClass('layui-this');
						$('.layui-nav .layui-nav-item').removeClass('layui-nav-itemed');
						othisx.parents('.layui-nav-item').addClass('layui-nav-itemed');
					}
				}
			}
		...
		, serach: function (othis) {
			othis.off('keypress').on('keypress', function (e) {
				...
				if (e.keyCode === 13) {
					...
					text = text + ' <span style="color: #E8001C;">' + admin.escape(this.value) + '</span>';
	...
	!function () {
        ...
        var strIe = DEFAULT_LANG == 'us'?'Access under IE' + device.ie + ' may be poor, recommended: Chrome / Firefox / Edge and other advanced browsers':'IE' + device.ie + '下访问可能不佳，推荐使用：Chrome / Firefox / Edge 等高级浏览器'
		//低版本IE提示
				...
				list.each(function (index1, item1) {
					...
					listChildren1.each(function (index2, item2) {
						...
						if (matched2) {
							...
							$('.layui-nav .layui-nav-item').removeClass(SIDE_NAV_ITEMD);
