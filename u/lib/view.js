...
layui.define(['laytpl', 'layer'], function(exports) {
	...
	view.loading = function(elem) {
		var st = DEFAULT_LANG=='us'?'Loading...':'正在加载...'
		elem.append(
			$('<div class="loading-box layadmin-loading" id="elemLoad"><i class="layui-anim layui-anim-rotate layui-anim-loop layui-icon layui-icon-loading"></i><span>'+st+'</span></div>')
		);
	...
	view.req = function(options) {
		...
		return $.ajax($.extend({
			...
			success: function(res) {
				...
				switch (res[response.statusName]) {
					case statusCode.ok: // 成功
						typeof options.done === 'function' && options.done(res);
						break;
					case statusCode.logout: // 退出登录
						layui.x.islogin();
						break;
					case statusCode.bh_code: //需要解除保护
						layui.x.mmbh_tc(res);
						break;
					case statusCode.bh_wsz: //需要解除保护但保护未设置
						layui.x.mmbh_wsz(res);
						break;
					case statusCode.update_token: // 获取token
						layui.x.get_token();
						break;
					case statusCode.update_yzm: // 获取验证码
						$("#re_yzmx,#re_yzm,#re_yzms").trigger('click');
						break;
                    case statusCode.noqx: // 无权限
                        layer.msg("无权限");
                        // setTimeout(function () {
	...
	Class.prototype.parse = function(html, refresh, callback) {
		var that = this,
			isScriptTpl = typeof html === 'object' //是否模板USD素
			,
